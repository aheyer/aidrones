...
\begin{description}
	\item[NAV ALTHOLD] ist ein solcher Modus. Dieser setzt lediglich die Flughöhe fest und muss in Kombination mit einem manuellen Flugmodus verwendet werden. Hierbei ist ANGLE empflohlen, da ALTHOLD nicht für Überkopf-Maneuver geeignet ist. Im "Advanced Tuning" Tab kann mit "Max. Alt-hold climb rate" der Spielraum konfiguriert werden, mit welchem der Pilot die Höhe manuell nach oben und unten variieren kann.
...
\begin{description}
	...
	\item[WP PLANNER] ist kein eigenständiger Flugmodus, sondern dient dazu Wegpunkte während des manuellen Flugs zu definieren. WP darf nicht aktiviert sein, sonst kann kein Wegpunkt gespeichert werden. Darüber hinaus muss der Block MISSION INFO im OSD aktiviert werden. Dieser Block zeigt die für den Planner notwendigen Informationen an. Um einen Wegpunkt zu speichern, muss die Drohne an den gewünschten Ort bewegt und der Switch auf dem WP PLANNER konfiguriert ist betätigt werden. Hierbei ist zu beachten, dass die Höhe entsprechend mitgespeichert wird. Dies ist wichtig, da ein Wegpunkt der ebenerdig gespeichert wurde auch entsprechend angeflogen wird. Um die so konfigurierte Mission zu fliegen, genügt es PLANNER zu deaktivieren und WP zu aktivieren. Alternative kann die Mission auch mit Stick-Commands gespeichert werden (siehe Kapitel xxx)
\end{description}
...
Nach der Konfiguration des Advanced Tuning Tabs kommt es nun zum eigentlichen Setzen der Wegpunkte. Die Wegpunkte können durch einfaches Klicken auf der Karte gesetzt und aneinander gereiht werden.
...
Beim Wegpunkttypen besteht die Auswahl zwischen Waypoint, Position-Hold (PH), Point-of-Interest (POI) und Land. Der Waypoint beschreibt einen einfachen Wegpunkt, durch den die Drohne hindurch fliegt. PH beschreibt einen Wegpunkt an dem die Drohne für eine bestimmte Zeit anhält, welche im Feld Wait time konfiguriert wird. POI ist Punkt in der Mission an dem sich die Ausrichtung der Drohne im Flug orientiert. Wird ein Wegpunkt als POI definiert, orientiert sich die Kamera an diesem anstatt sich am Weg auszurichten, während sie zum nächsten Wegpunkt fliegt. Land definiert einen Wegpunkt an dem die Drohne landet, statt die Mission fortzuführen.

Bei den Actions hat man JUMP, SET HEAD und RTH zur Verfügung. Mit Jump kann zu einem anderen Wegpunkt gesprungen werden, als dem mit der nächstgrößeren Nummer. In P1 kommt dazu die Nummer des anzufliegenden Wegpunkts und in P2 die Anzahl der Wiederholungen.
Mit SET HEAD kann die Ausrichtung der Drohne gesteuert werden, sobald sie einen gewissen Wegpunkt erreicht. RTH lässt die Drohne zum konfigurierten Safehome zurückfliegen, sobald die Drohne am Wegpunkt angekommen ist.

Darüber hinaus kann die Höhe eingestellt werden. Über den Button MP Elevation kann das Höhenprofil für die Mission, mit dem Höhenprofil der Umgebung verglichen werden. Damit kann vermieden werden, dass die Drohne während der Mission mit dem Boden kollidiert. Um diesen Vergleich zu ermöglichen, muss jedoch eine Form von Take-Off-Location vorhanden werden.
...
\begin{figure}[htb!]
	...
	\caption{Stick Commands in INAV, Source: https://raw.githubusercontent.com/iNavFlight/inav/refs/heads/master/docs/assets/images/StickPositions.png}
	\label{INAV_Stick_Commands}
\end{figure}


% NAV WP
% 1. NAV WP muss konfiguriert werden.
% 2. Advanced tuning options (incl. 1-2 examples)
% 3. Creating the Mission in Mission Control
%   1. Setting the Waypoints
%   2. Setting the altitude in accordance with ground level
%   3. Setting the waypoint type 
% 4. Saving and Loading the Missions 
% 5. Flying the missions
