...
\renewcommand{\englischertitel}{Object Detection by additional Hardware on the Drone}
\makroabschnitt
...
\toprule
Generation   & CPU Cores   & \multicolumn{1}{c}{RAM}       & \multicolumn{1}{c}{Size}    &  \multicolumn{1}{c}{Weight}  & \multicolumn{1}{c}{Current Draw} & \multicolumn{1}{c}{Current Draw} \\
                       &               &     &      &      & \multicolumn{1}{c}{(idle)} &  \multicolumn{1}{c}{(peak)} \\
\midrule
Pi Zero W    & 1\,\(@\)\,1000\,MHz  & 512\,MB  & 65x30x5\,mm   & \(\sim 9\)\,g  & \(\sim\)\,120\,mA & \(\sim\)\,350\,mA \\
Pi Zero 2 W  & 4\,\(@\)\,1000\,MHz  & 512\,MB  & 65x30x5\,mm   & \(\sim 10\)\,g & \(\sim\)\,140\,mA & \(\sim\)\,600\,mA \\
Pi 3 B+      & 4\,\(@\)\,1200\,MHz  & 1\,GB    & 85x56x16\,mm  & \(\sim 50\)\,g & \(\sim\)\,500\,mA & \(\sim\)\,1400\,mA \\
Pi 4 B       & 4\,\(@\)\,1500\,MHz  & 1-8\,GB  & 85x56x16\,mm  & \(\sim 50\)\,g & \(\sim\)\,600\,mA & \(\sim\)\,1500\,mA \\
Pi 5         & 4\,\(@\)\,2400\,MHz  & 2-16\,GB & 85x56x16\,mm  & \(\sim 70\)\,g & \(\sim\)\,700\,mA & \(\sim\)\,2500\,mA \\
\bottomrule
\end{tabular}
\end{table}
\FloatBarrier
...
\toprule
AI Hardware                     & Interface       & TOPS        & Weight           & Size [mm] & Current Draw  & \multicolumn{1}{c}{Support} \\
\midrule
Google Coral TPU                & USB 3.0         & 4           & \(\sim\)\,20\,g  & 65x30x8   & 500-900\,mA & Discontinued  \\
Intel NCS\,2                    & USB 3.0         & 4           & \(\sim\)\,30\,g  & 73x28x14  & 200-500\,mA & Discontinued  \\
RPi AI Camera                   & CSI             & ?           & \(\sim\)\,6\,g   & 25x24x10  & 300-600\,mA & Active  \\
RPi AI\,Kit/HAT+                & HAT (PCIe)      & 13          & \(\sim\)\,40\,g  & 65x57x6   & 400-800\,mA & Active  \\
\bottomrule
\end{tabular}
\end{table}
\FloatBarrier
...
Die in Tabelle~\ref{TabelleDrohne35GoogleCoral} aufgeführten Komponenten betreffen ausschließlich die notwendigen Bestandteile der Drohne selbst. Weiteres erforderliches Zubehör wie Fernsteuerung (80-160\,\euro{}), FPV-Brille (120-250\,\euro{}) und Ladegerät (50-100\,\euro{}) muss bereits vorhanden sein oder zusätzlich beschafft werden. Die Preise der einzelnen Komponenten hängen auch von persönlichen Präferenzen und finanziellen Möglichkeiten ab.
...
\begin{figure}[htb!]
	\centering
	\includegraphics[width=\linewidth]{Object_Detection_using_USB_video_grabber.png}
	\caption{Object Detection using a USB Video Grabber connected to a FPV Goggles}
	\label{ObjectDetectionDashboardUsingUSBVideoGrabber}
\end{figure}
\FloatBarrier

\colstart

Once the live analog video signal from the FPV goggles is digitized using a USB video grabber and transmitted to a ground-based computing system, a wide range of real-time video processing techniques can be applied. A practical and widely adopted solution for this purpose is the OpenCV (Open Source Computer Vision) library, which provides efficient tools for real-time image acquisition, processing, and analysis using Python.

\colger

Sobald das analoge Live-Videosignal der FPV-Brille mithilfe eines USB-Videograbbers digitalisiert und an ein bodengebundenes Rechensystem übertragen wurde, können vielfältige Verfahren zur Echtzeit-Bildverarbeitung angewendet werden. Eine praxisnahe und weit verbreitete Lösung stellt die Bibliothek OpenCV (Open Source Computer Vision) dar, die effiziente Werkzeuge zur Echtzeit-Bildaufnahme, -verarbeitung und -analyse unter Python bereitstellt.

\coleng

The digitized video feed is treated as a standard camera input by the operating system, allowing OpenCV to capture frames in real time. This approach makes the system platform-independent and suitable for Linux, macOS, and Windows environments.

\colger

Der digitalisierte Videostream wird vom Betriebssystem als gewöhnliche Kameraquelle behandelt, wodurch OpenCV die Einzelbilder in Echtzeit erfassen kann. Dieser Ansatz ist plattformunabhängig und eignet sich für Linux-, macOS- und Windows-Systeme.

\coleng

After acquiring the live video frames, various computer vision and object detection algorithms can be applied, including but not limited to:

\colger

Nachdem die Live-Videobilder erfasst wurden, können verschiedene Verfahren der Bildverarbeitung und Objekterkennung angewendet werden. Dazu zählen unter anderem:

\coleng

\begin{itemize}
  \item Region or area-based detection
  \item Detection of other drones or aerial objects in the sky
  \item Obstacle detection and tracking
  \item Motion analysis and situational awareness
\end{itemize}

\colger

\begin{itemize}
  \item regions- oder flächenbasierte Erkennung
  \item Erkennung anderer Drohnen oder fliegender Objekte im Luftraum
  \item Hinderniserkennung und -verfolgung
  \item Bewegungsanalyse und Situationsbewertung
\end{itemize}

\coleng

In addition to visual information, FPV video streams can contain embedded telemetry data such as altitude, speed, GPS position, battery voltage, and the current flight mode. This telemetry data can be extracted and synchronized with visual detections. This enables advanced applications such as geo-referenced object detection, situation assessment, and flight-state-aware decision making.

\colger

Zusätzlich zu den visuellen Informationen können FPV-Videostreams eingebettete Telemetriedaten enthalten, beispielsweise Höhe, Geschwindigkeit, GPS-Position, Akkuspannung und den aktuellen Flugmodus. Diese Telemetriedaten lassen sich extrahieren und mit visuellen Erkennungen synchronisieren. Dadurch werden weiterführende Anwendungen wie georeferenzierte Objekterkennung, Lagebewertung und flugzustandsabhängige Entscheidungsfindung ermöglicht.
...
\begin{figure}
	\begin{lstlisting}[style=shell]
  camera = cv2.VideoCapture(CAMERA_INDEX)  # Linux and macOS
  camera = cv2.VideoCapture(CAMERA_INDEX, cv2.CAP_DSHOW)  # Windows

  success, frame = camera.read()
	\end{lstlisting}
	\caption{Reading Video Stream from a Video Grabber using OpenCV in Python}
	\label{ReadingUSBVideoStreamMitOpenCV}
\end{figure}

\colstart

Figure~\ref{ReadingUSBVideoStreamMitOpenCV} shows an example of how a video stream from a USB video grabber can be accessed using OpenCV in Python. The device index specifies the camera input as recognized by the operating system. In many cases, index 0 is used when no additional camera devices are connected. On systems with multiple video sources, a different index may be required. On Windows systems, it is recommended to explicitly use the DirectShow backend by specifying \verb!cv2.CAP_DSHOW! to ensure stable access to USB video grabbers.

\colger

Abbildung~\ref{ReadingUSBVideoStreamMitOpenCV} zeigt ein Beispiel für den Zugriff auf einen Videostream eines USB-Videograbbers mit OpenCV in Python. Der Geräteindex gibt dabei die vom Betriebssystem erkannte Kameraquelle an. In vielen Fällen wird der Index 0 verwendet, sofern keine weiteren Kamerageräte angeschlossen sind. Bei Systemen mit mehreren Videoquellen kann ein anderer Index erforderlich sein. Unter Windows wird empfohlen, explizit das DirectShow-Backend mit \verb!cv2.CAP_DSHOW! zu verwenden, um einen stabilen Zugriff auf USB-Videograbber sicherzustellen.
...
\begin{figure}
  ...
  CAMERA_INDEX = 0         # SET YOUR CAMERA INDEX HERE
  MODEL_PATH = "models/final_drone_model.pt"
  IMG_SIZE = (640, 480)

  # Open camera (DirectShow is REQUIRED on Windows)
  ...
  print("YOLO loaded. Press CRL + c to exit.")
  while True:
  ...
  cv2.destroyAllWindows()
	...
	\end{lstlisting}
	\caption{Real-time drone detection using a USB video grabber on the ground system}
	\label{DroneDetectionUSBVideoGrabber}
\end{figure}

\colstart

Figure~\ref{DroneDetectionUSBVideoGrabber} shows a minimal example that demonstrates real-time drone detection using a USB video grabber and OpenCV. In this example, a pretrained YOLO model is used that is capable of detecting drones in aerial scenes. The model was trained on a dedicated drone detection dataset and is publicly available at \url{https://www.kaggle.com/code/zlatan599/drone-detection/output}. After the live video frames are captured with OpenCV, they are passed to the YOLO model for inference. Detected objects are then visualized using bounding boxes and associated confidence scores.

\colger

Abbildung~\ref{DroneDetectionUSBVideoGrabber} zeigt ein minimales Beispiel zur Echtzeit-Erkennung von Drohnen mithilfe eines USB-Videograbbers und OpenCV. In diesem Beispiel wird ein vortrainiertes YOLO-Modell eingesetzt, das für die Erkennung von Drohnen in Luftaufnahmen geeignet ist. Das Modell wurde mit einem speziellen Datensatz zur Drohnenerkennung trainiert und ist öffentlich unter \url{https://www.kaggle.com/code/zlatan599/drone-detection/output} verfügbar. Nachdem die Live-Videoframes mit OpenCV erfasst wurden, werden sie zur Inferenz an das YOLO-Modell übergeben. Erkannte Objekte werden anschließend durch Begrenzungsrahmen und zugehörige Konfidenzwerte visualisiert.
...
Tabelle~\ref{TabelleDrohne35Videograbber} enthält die Komponenten der Drohne aus Tabelle~\ref{TabelleDrohne35GoogleCoral}, reduziert um die Hardware, die für die Objekterkennung erforderlich ist. Konkret sind das der Raspberry Pi Zero, das Kameramodul, die Google Coral TPU sowie die entsprechenden Kabel und Gehäuse. Weiteres notwendiges Zubehör wie Fernsteuerung, FPV-Brille und Ladegerät muss selbstverständlich ebenfalls bereits vorhanden sein oder zusätzlich beschafft werden. Zusätzlich werden ein Videograbber (15–25 \euro{}) und ein Computer benötigt.
