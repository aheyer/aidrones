...
In addition to visual data, FPV video streams can be embedded with telemetry information, such as altitude, speed, GPS coordinates, battery voltage, and flight mode. This telemetry data can be extracted and synchronized with visual detections, enabling more advanced applications such as geo-referenced object detection, threat assessment, and flight-state-aware decision making.
...
Figure~\ref{ReadingUSBVideoStreamMitOpenCV} shows an example of how to read a video stream from a USB video grabber using OpenCV in Python. The device index corresponds to the camera input number recognized by the operating system. Typically, index 0 is used when no other camera devices are present; however, systems with multiple cameras may require a different index. On Windows systems, it is recommended to explicitly use the DirectShow backend (cv2.CAP\_DSHOW) to ensure reliable access to USB video grabbers.
...
Figure~\ref{DroneDetectionUSBVideoGrabber} is a minimal example demonstrates real-time drone detection using a USB video grabber and OpenCV. In this example, a pretrained YOLO model capable of detecting drones in aerial scenes is used. The model has been trained on a drone detection dataset and is publicly available here: \url{https://www.kaggle.com/code/zlatan599/drone-detection/output}. Once the live video frames are captured using OpenCV, they are passed to the YOLO model for inference, and detected objects are visualized using bounding boxes and confidence scores.
