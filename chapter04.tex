...
An easy way to access the live video feed is by using the AV interface that some FPV goggles (see Table~\ref{TabelleVideobrillen}) provide. The video signal can be digitized with a video grabber and transmitted to a computer for further processing.

\colger

Eine einfache Möglichkeit, auf das Livebild zuzugreifen, bietet die gegebenenfalls vorhandene AV-Schnittstelle der Videobrille (siehe Tabelle~\ref{TabelleVideobrillen}). Das Videosignal kann mithilfe eines Videograbbers digitalisiert und zur Verarbeitung an einen Computer weitergeleitet werden.
