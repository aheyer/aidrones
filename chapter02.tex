...
If the flight controller is not being flashed for the first time, it is good practice to create a backup of the configuration, as some upgrades reset it or to be save in case of a failure. This is done via the \textsl{CLI} tab. The command \texttt{diff all} outputs the corresponding parameters. These can then be saved to a text file using the \textsl{Save to File} button. The flashing process itself consists of the following steps:
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item \textsl{Manual baud rate} - Set a manual baud rate for Bluetooth or for USB if a model does not support the standard speed.
	\end{itemize}
\end{enumerate}
...
\begin{enumerate}
	...
	\item Im \textsl{Modes-Tab}, welcher der Betaflight-Version ebenfalls sehr ähnlich ist (Abbildung \ref{AbbildungBetaflightAppModes}) muss grundsätzlich nur \texttt{ARM} konfiguriert werden (CH5 empfohlen). Darüber hinaus lohnt es sich auch \texttt{ANGLE} zu konfigurieren. Dies ist vor allem für Fluganfänger interessant, da Pitch und Roll limitiert werden, was das Fliegen vereinfacht. \texttt{FAILSAFE} ist auch zu empfehlen, um reagieren zu können, sollte die Drohne unerwartet verhalten. Modi für Autopilotfunktionalitäten können zu einem späteren Zeitpunkt konfiguriert werden. 
\end{enumerate}
