...
Darüber hinaus werden intrinsischen und extrinsischen Kameraparameter berechnet. Die intrinsischen Parameter beziehen sich auf die Einstellungen der Kamera ($K$). Diese sind unabhängig von der betrachteten Szenerie und können nach der ersten Berechnung wiederverwendet werden, solange mit diesen Einstellungen gearbeitet wird.  Bei den Einstellungen handelt es sich um die Brennweite $(f_x, f_y)$ und das optische Zentrum $(c_x, c_y)$, jeweils als Pixelwert.
...
Bei den extrinsischen Parametern handelt es sich um die 3x3 Rotationsmatrix und den dreidimensionalen Übersetzungsvektor ($R$ und $t$), welche die Rotation und Position der Kamera in der Umwelt beschreiben. Diese können dann zu einer homogenen Transformation kombiniert werden, welche die Übersetzung des Punktes in der realen Welt ($P_w$) zu einem Punkt im Koordinatensystem der Kamera ($P_c$) beschreibt.
...
Um zu beschreiben wo der Punkt auf dem Bild ($P_i$) liegt und nicht nur auf dem Koordinatensystem der Kamera ($P_c$) muss dieser auch mit der intrinsischen Kameramatrix $K$ multipliziert werden.
...
Die Kalibrierung erfolgt mithilfe eines Kalibrierungsmusters. Hierbei besteht die Auswahl zwischen einem \textsl{Schachbrettmuster}, einem \textsl{Raster aus Kreisen} und einem sogennanten \textsl{ChArUco-Board} (Abbildung~\ref{ImageStereoVisionCharucoBoard}). Letzteres ist ein Schachbrettmuster, welches mit Aruco-Markierungen versehen wurde.
...
Bei den anderen beiden Pattern müssen alle Ecken, bzw. Kreise im Bild zu sehen sein. Dies macht es besonders schwierig die Ecken der Aufnahmen abzudecken, welche oft kritisch sind für eine gute Genauigkeit. Des weiteren werden dabei mehr Bilder benötigt. Wie viele Bilder konkret benötigt werden, sollte im Einzelfall getestet werden. \textsl{Zehn} sind grundsätzlich ein guter Startpunkt, da diese mindestens für die Kalibrierung mit einem Schachbrettmuster benötigt werden. Die anderen Muster können gegebenenfalls auch mit weniger auskommen.
...
Als letzter Schritt vor der Kalibrierung müssen die Bilder der beiden Kameras ausgewertet werden. Dies geschieht mit Hilfe von \texttt{cv2.aruco.CharucoDetector(board)} (Abbildung~\ref{ImageStereoVisionCharucoDetections}). Dieser wird verwendet, um die Ecken des Schachbretts und die entsprechnden Aruco-Markierungen zu erkennen und als Zahlenwerte zu beschreiben. Diese Zahlenwerte werden dann als Input für \texttt{cv2.calibrateCamera()} verwendet (Abbildung~\ref{ListingCornerDetection}).
...
Nach der Kalibrierung kann die intrinsische Kameramatrix mit \texttt{cv2.getOptimalCameraMatrix()} verfeinert werden (Abbildung~\ref{ListingCameraCalibration}). Der dazu verwendete Skalierungsfaktor $\alpha = [0, 1]$ bestimmt wie viele ungewollte Pixel im Bild übrig bleiben. Wenn $\alpha = 1$ werden alle Pixel des ursprünglichen Bilds behalten. Dadurch entstehen jedoch einige Bereiche mit schwarzen Pixeln. Wenn $\alpha = 0$, dann werden diese Bereiche auf ein minimum reduziert, dadurch wird jedoch unter Umständen nicht die Vollständige Information des ursprünglichen Bilds bewahrt.
...
Mithilfe dieser Matrizen kann die Funktion \texttt{cv2.stereoRectify()} angewendet werden, um die epipolaren Linien horizontal zu machen (Abbildung ~\ref{ListingStereoRectification}). Dadurch haben beide Bilder die gleichen Y-Koordinaten, auch wenn sie nicht perfekt horizontal montiert sind. Dadurch wird die Tiefenerkennung deutlich robuster, da alle Punkte in Bild 1 auch in Bild 2 gefunden werden können. Die Resultate der Korrektur beinhalten die Korrektur- und Projektionsmatrizen, sowie die Matrix $Q$ und die jeweilige ROI der beiden Bilder.

In diesem Fall beschreibt die ROI einen Bereich im Bild, in welchem alle Pixel valide sind. Die Korrekturmatrizen beschreiben, wie die jeweiligen Bilder angepasst werden müssen, um horizontale Epipolarlinien zu erhalten. Die Projektionsmatrizen ($P_0, P_1$) beschreiben die notwendige Transformation, um einen 3D-Punkt auf das jeweils andere 2D-Koordinatensystem zu projizieren, unabhängig der Szenerie. Diese stellen sich dar, wie eine Erweiterung der intrinsischen Kameramatrix $K$.
...
Die Matrix $Q$ beschreibt eine \textsl{Reprojizierungsmatrix}. Wie diese verwendet werden kann, wird in Abschnitt \ref{AbschnittStereoVisionTiefenerkennung} erläutert.
...
Da die beiden Kameras statisch montiert sind, genügt es die Rectification einmal zu berechnen und dann zu speichern. Dadurch kann viel Rechenzeit gespart werden, wodurch die Performance verbessert wird. Dazu kann die Funktion \texttt{cv2.initUndistortRectifyMap()} verwendet werden (Abbildung~\ref{ListingInitUndistortRectifyMap}).
...
Nun da die Bilder auf eine Y-Achse gebracht werden können gilt es die Unterschiede zwischen den aufgenommenen Bildern zu berechnen. Hierzu wird der \textsl{Block-Matching (BM)-Algorithmus} verwendet. Dieser teilt das Bild in kleinere Blöcke (z.B. 5x5 Pixel) auf und sucht nach korrespondierenden Blöcken entlang der Epipolaren Linie im Referenzbild.

Anhand unterschiedlicher konfigurierbarer Metriken wie die Summe der absoluten Differenzen, die Summe der quadrierten Differenzen oder der normalisierten Kreuzkorrelation werden die Blöcke nacheinander verglichen und eine sogenannte \textsl{Disparity-Map} erstellt werden.

Diese Disparity-Map repräsentiert die Verschiebung zwischen den beiden Bildern innerhalb dieser Blöcke (beispielsweise als Grauwert). OpenCV bietet die Klasse \texttt{cv2.StereoBM} an welche verwendet werden kann, um eine solche Disparity-Map zu erstellen.

Alternativ kann auch \textsl{Semi-Global-Block-Matching (SGBM)} verwendet werden, um die Disparity-Map zu erstellen. Hierzu existiert ebenfalls eine OpenCV-Klasse. Diese trägt den Namen \texttt{cv2.StereoSGBM}. SGBM ermöglicht Sub-Pixel-Berechnungen, welche Texturen schärfer erscheinen lassen können.

Sowohl BM als auch SGBM können mit einer Vielzahl an Parametern verfeinert werden. Dies beinhaltet beispielsweise die Blockgröße die geprüft wird, oder ob ein Pre-Filter verwendet werden soll, um Texturen im Bild besser sichtbar zu machen. Die Methode \texttt{compute(img\_l, img\_r)}, welche die tatsächliche Berechnung der Disparity durchführt gibt Werte zurück, die mit 16 skaliert wurden. Um die tatsächlichen Disparity-Werte zu erhalten muss das Ergebnis durch 16 geteilt werden.
