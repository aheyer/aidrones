...
retS, new_mtx_zero, dist_zero, new_mtx_one, dist_one, Rot, Trns, Emat, Fmat = cv2.stereoCalibrate(obj_pts_one, img_pts_zero, img_pts_one, mtx_zero, dist_zero, mtx_one, dist_one, (w_zero,h_zero), criteria_stereo, cv2.CALIB_FIX_INTRINSIC)
	\end{lstlisting}
	\caption{Single Camera calibration}
...
Mithilfe dieser Matrizen kann die Funktion \texttt{cv2.stereoRectify()} angewendet werden, um die epipolaren Linien horizontal zu machen. Dadurch haben beide Bilder die gleichen Y-Koordinaten, auch wenn sie nicht perfekt horizontal montiert sind und die Tiefenerkennung wird deutlich robuster. Sie wird robuster, da alle Punkte in Bild 1 auch in Bild 2 gefunden werden können.
